...
- **The "loaded set" is tracked per-PC** in `chrome.storage.local` (not sync storage), so each machine tracks its own working state independently. A machine that has no marker yet (fresh install, second PC) first tries to recognise the toolbar by **content**; if nothing matches, the bar is **unrecognised** — see below.
- **The toolbar is never wiped without a baseline.** A switch destroys the toolbar, so it is guarded by two questions, not one: *dirty* (drifted from a known set → save the diff) and *unrecognised* (matches no set at all, so nothing on it is stored anywhere → the popup offers **Save as a new bar**, and the hotkey refuses the switch and raises a badge rather than erasing silently).
- **A common bar merges folder-by-folder** into each bar that opts into it; the common itself stays read-only.

**Where the model stops — two machines browsing at the same time.** The toolbar is an ordinary synced bookmark folder, so two machines each holding a different set on it are writing to the same synced node, and the browser will merge them. No extension can prevent that: sync-applied changes arrive as ordinary `chrome.bookmarks` events, after the fact and indistinguishable from the user's own edits — there is no pre-merge hook to hold anything back. What the model buys is where the collision lands: on the disposable toolbar, never in a master (which only an explicit save rewrites). The merged toolbar is surfaced as **unsaved changes** where a baseline exists, and as an **unrecognised bar** where one doesn't — either way the next switch asks before erasing it. Recovery is a switch away with **Discard** and a switch back; the failure to warn users about is saving the merged toolbar over a good set.

**Multi-machine rule of thumb:** save before you leave a machine. Nothing is lost if you don't — an unsaved toolbar is now recognised as unrecognised and the switch stops to ask — but saving is what lets the other machine pick up exactly where you left off instead of prompting.
